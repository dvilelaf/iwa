...
@pytest.fixture
def mock_secrets():
    with patch("iwa.protocols.gnosis.safe.Secrets") as mock:
...
@pytest.fixture
def mock_safe_lib():
    with patch("iwa.protocols.gnosis.safe.Safe") as mock_safe, \
         patch("iwa.protocols.gnosis.safe.EthereumClient") as mock_client:
...
def test_send_tx(mock_safe_account, mock_secrets, mock_safe_lib):
    ...
    safe.send_tx(
        to="0xTo",
        value=100,
        signers_private_keys=["key1", "key2"],
        data="0x1234"
    )
