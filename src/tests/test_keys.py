...
def test_keystorage_init_existing(mock_secrets):
    ...
    with patch("os.path.exists", return_value=True), \
        ...
        assert isinstance(storage.accounts["0x1111111111111111111111111111111111111111"], EncryptedAccount)
...
def test_keystorage_create_account_duplicate_tag(mock_secrets, mock_account, mock_aesgcm, mock_scrypt):
    storage = KeyStorage(Path("wallet.json"))
    with patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_create_safe(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    storage = KeyStorage(Path("wallet.json"))
...
def test_keystorage_get_private_key(mock_secrets, mock_account, mock_aesgcm, mock_scrypt):
    ...
    with patch("builtins.open", mock_open()), \
         ...
         patch("pathlib.Path.mkdir"):
        storage.create_account("tag")
        pkey = storage.get_private_key("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4")
        assert pkey == "private_key"

def test_keystorage_get_private_key_safe_error(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    storage = KeyStorage(Path("wallet.json"))
    with patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
        ...
        with pytest.raises(ValueError, match="Cannot get private key"):
            storage.get_private_key("0x61a4f49e9dD1f90EB312889632FA956a21353720")

def test_keystorage_get_safe_signer_keys(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    storage = KeyStorage(Path("wallet.json"))
    with patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_create_account_default_tag(mock_secrets, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_create_safe_owner_not_found(mock_secrets, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_get_private_key_not_found(mock_secrets):
    with patch("os.path.exists", return_value=False):
        storage = KeyStorage(Path("wallet.json"))
        assert storage.get_private_key("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4") is None
...
def test_keystorage_get_safe_signer_keys_not_enough_signers(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_get_account_safe(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
...
def test_keystorage_redeploy_safes(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
        ...
        with patch("iwa.core.keys.KeyStorage.create_safe", wraps=storage.create_safe) as mock_create_safe:
             storage.redeploy_safes()
             assert mock_create_safe.call_count > 0

def test_keystorage_redeploy_safes_already_deployed(mock_secrets, mock_safe, mock_ethereum_client, mock_account, mock_aesgcm, mock_scrypt):
    with patch("os.path.exists", return_value=False), \
         patch("builtins.open", mock_open()), \
         patch("os.chmod"), \
         patch("pathlib.Path.mkdir"):
