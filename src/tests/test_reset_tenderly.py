...
from iwa.core.models import TenderlyConfig, VirtualNet, FundRequirements, TokenAmount
...
def mock_tenderly_config():
    ...
    vnet.funds_requirements = {
        "tag1": FundRequirements(native=1.0, tokens=[TokenAmount(address="0xToken", amount=10.0, symbol="TKN")])
    }
...
def test_create_vnet(mock_requests):
    ...
    vnet_id, admin_rpc, public_rpc = _create_vnet(
        "key", "account", "project", 1, 1, "slug", "name"
    )
...
def test_fund_wallet_json_error(mock_requests, capsys):
    import requests
    mock_requests.post.return_value.status_code = 500
    mock_requests.post.return_value.json.side_effect = requests.exceptions.JSONDecodeError("msg", "doc", 0)
...
def test_main(mock_requests, mock_tenderly_config):
    ...
    with patch.dict(os.environ, {
        ...
        with patch("iwa.core.models.TenderlyConfig.load", return_value=mock_tenderly_config):
            # Mock KeyStorage
            with patch("iwa.tools.reset_tenderly.KeyStorage") as MockKeyStorage:
                mock_keys = MockKeyStorage.return_value
                ...
                mock_response_create.json.return_value = {
                    "id": "new_id",
                    "rpcs": [{"name": "Admin RPC", "url": "admin"}, {"name": "Public RPC", "url": "public"}]
                }
