...
class MechManagerMixin:
    ...
    def send_batch_mech_requests(  # noqa: C901
        ...
        """Send multiple mech requests in a single Safe multi-send transaction.

        This batches N marketplace mech requests into ONE Safe transaction via
        the MultiSend contract, reducing total time from ~N minutes to ~10 seconds.

        Only supports marketplace v2 requests (not legacy mech).
        The sender MUST be a Safe multisig (multi-send uses DELEGATE_CALL).

        Args:
            data_list: List of request data payloads (IPFS hash bytes), one per request.
            priority_mech: Priority mech address. Auto-detected if None.
            marketplace_address: Marketplace contract address. Auto-detected if None.
        ...
        count = len(data_list)
        if count == 0:
            logger.error("[MECH-BATCH] No requests to send")
            return None

        if not self.service:
            logger.error("[MECH-BATCH] No active service loaded")
            return None

        multisig_address = self.service.multisig_address
        if not multisig_address:
            logger.error(f"[MECH-BATCH] Service {self.service.service_id} has no multisig")
            return None

        # Verify sender is a Safe
        from iwa.core.models import StoredSafeAccount

        sender_account = self.wallet.account_service.resolve_account(str(multisig_address))
        if not isinstance(sender_account, StoredSafeAccount):
            logger.error("[MECH-BATCH] Multi-send only supported for Safe multisigs")
            return None

        # Auto-detect marketplace config
        if marketplace_address is None or priority_mech is None:
            use_mp, detected_mp, detected_pm = self.get_marketplace_config()
            if not use_mp:
                logger.error(
                    "[MECH-BATCH] Service does not use marketplace flow; "
                    "batch requests only support marketplace v2"
                )
                return None
            marketplace_address = marketplace_address or detected_mp
            priority_mech = priority_mech or detected_pm
        ...
        try:
            ...
            logger.error(f"[MECH-BATCH] {e}")
            return None

        if marketplace_address in DEFUNCT_MARKETPLACES:
            logger.error(
                f"[MECH-BATCH] V1 marketplace {marketplace_address} is defunct"
            )
            return None
        ...
        if not self._validate_marketplace_params(marketplace, 300, payment_type):
            return None

        # Build individual transaction calldata for each request
        inner_txs = []
        for i, data in enumerate(data_list):
            ...
            if not tx_data:
                logger.error(f"[MECH-BATCH] Failed to prepare request {i + 1}/{count}")
                return None

            inner_txs.append({
                "operation": SafeOperationEnum.CALL,
                "to": str(marketplace_address),
                "value": int(tx_data.get("value", 0)),
                "data": bytes.fromhex(tx_data["data"][2:])
                if isinstance(tx_data["data"], str) and tx_data["data"].startswith("0x")
                else tx_data["data"],
            })

        logger.info(
            f"[MECH-BATCH] Prepared {count} marketplace requests, "
            f"total value: {sum(tx['value'] for tx in inner_txs)} wei"
        )

        # Build multi-send transaction
        multi_send_contract = MultiSendContract(
            address=MULTISEND_ADDRESS, chain_name=self.chain_name
        )
        multi_send_tx = multi_send_contract.prepare_tx(
            from_address=multisig_address,
            transactions=inner_txs,
        )
        if not multi_send_tx:
            logger.error("[MECH-BATCH] Failed to prepare multi-send transaction")
            return None

        # Execute via Safe (DELEGATE_CALL to MultiSend)
        logger.info(
            f"[MECH-BATCH] Sending {count} mech requests in single Safe multi-send TX"
        )
        try:
            tx_hash = self.wallet.safe_service.execute_safe_transaction(
                ...
                value=multi_send_tx["value"],
                chain_name=self.chain_name,
                data=multi_send_tx["data"],
                operation=SafeOperationEnum.DELEGATE_CALL.value,
            )
        except Exception as e:
            logger.error(f"[MECH-BATCH] Safe transaction failed: {e}")
            return None

        if not tx_hash:
            logger.error("[MECH-BATCH] No tx hash returned from Safe execution")
            return None

        logger.info(f"[MECH-BATCH] Multi-send transaction sent: {tx_hash}")
        ...
        try:
            ...
            if event_count == count:
                logger.info(
                    f"[MECH-BATCH] All {count} MarketplaceRequest events verified"
                )
            elif event_count > 0:
                logger.warning(
                    f"[MECH-BATCH] Only {event_count}/{count} MarketplaceRequest events found. "
                    f"Some inner calls may have failed."
                )
            else:
                logger.error(
            ...
            logger.error(f"[MECH-BATCH] Error verifying events: {e}")
            # Return tx_hash anyway - the TX was sent, we just couldn't verify
            return tx_hash
