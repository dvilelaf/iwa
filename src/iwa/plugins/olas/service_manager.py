...
class ServiceManager:
    ...
    def __init__(self, wallet: Wallet):
        ...
        # Retrieve OlasConfig from global Config
        global_config = Config()
        if not global_config.plugins or "olas" not in global_config.plugins:
            # Fallback or raise? For now, we assume plugin loader put it there, or we handle it gracefully if testing.
            # In testing we might need to mock Config.plugins

            # Or raise if strict.
            # raise ValueError("Olas plugin configuration not found")
            self.config = None
        else:
            self.config = global_config.plugins["olas"]

        self.services = self.config.services if self.config else None
    ...
    def register_agent(self) -> bool:
        ...
        agent_tag = f"service_{self.config.service_id}_agent"
        agent_account = self.wallet.key_storage.create_account(agent_tag)

        # Fund the agent
        success, receipt = self.wallet.send(
            from_address_or_tag=self.wallet.master_account.address,
            to_address_or_tag=agent_account.address,
