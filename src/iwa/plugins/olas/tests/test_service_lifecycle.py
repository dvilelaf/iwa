...
from iwa.plugins.olas.service_manager import ServiceManager, AgentType
...
def sm(mock_wallet):
    with patch("iwa.core.models.Config"), \
        ...
        mock_ci.get_instance.return_value.get.return_value.chain.get_token_address.side_effect = lambda x: x
...
def test_sm_create_utility_not_found(sm):
    ...
    with patch("iwa.plugins.olas.constants.OLAS_CONTRACTS", {"gnosis": {}}):
        ...
        sm.registry.extract_events = MagicMock(return_value=[{"name": "CreateService", "args": {"serviceId": 42}}])
...
def test_sm_create_approve_fail(sm):
    ...
    sid = sm.create("gnosis", "test", token_address_or_tag="0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB")
    assert sid == 42
...
def test_sm_stake_fail(sm):
    ...
    with patch("iwa.plugins.olas.contracts.staking.StakingContract") as mock_stk_cls, \
         patch("iwa.plugins.olas.service_manager.ERC20Contract") as mock_erc20_cls:
        mock_stk = mock_stk_cls.return_value
...
def test_importer_safe_duplicate(importer):
    ...
    service = DiscoveredService(
        service_id=1, chain_name="gnosis", safe_address=addr
    )
