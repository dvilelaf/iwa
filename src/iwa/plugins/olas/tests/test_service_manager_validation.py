...
def test_call_checkpoint_success(sm):
    ...
    staking_mock.extract_events.return_value = [{"name": "Checkpoint", "args": {"epoch": 1, "availableRewards": 10**18}}]
...
def test_spin_up_intermediate_failure(sm):
    ...
    with patch.object(sm.registry, "get_service", side_effect=[
        ...
        with patch.object(sm, "activate_registration", return_value=True) as m1, \
