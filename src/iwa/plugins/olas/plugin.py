...
class OlasPlugin(Plugin):
    ...
    def import_services(
        ...
        yes: bool = typer.Option(False, "--yes", "-y", help="Import all without confirmation prompts"),
    ):
        ...
        for i, service in enumerate(discovered, 1):
            table = Table(title=f"Service {i}: {service.service_name or 'Unknown'}", show_header=False)
            ...
            if service.safe_address:
                on_chain_signers, safe_exists = self._get_safe_signers(service.safe_address, service.chain_name)
                ...
                if safe_exists is None:
                    ...
                    table.add_row("Safe", f"[bold red]⚠ {service.safe_address} - DOES NOT EXIST ON-CHAIN![/bold red]")
            else:
        ...
        needs_password = any(
            key.is_encrypted for service in discovered for key in service.keys
        )
