...
with patch("iwa.core.wallet.Wallet"), \
    ...
    from iwa.web.server import app, wallet
...
def test_get_state(client):
    ...
    with patch("iwa.core.chain.ChainInterfaces") as mock_chains:
        mock_chains.get_instance.return_value.chains = {
            "gnosis": MagicMock(name="gnosis")
        }
...
def test_get_accounts(client):
    ...
    wallet.key_storage.accounts = {
        "0x123": MagicMock(tag="test", address="0x123", is_safe=False)
    }
...
def test_get_rpc_status(client):
    ...
    with patch("iwa.web.server.ChainInterfaces") as mock_chains:
...
def test_create_safe_success(client):
    ...
    response = client.post("/api/accounts/safe", json={
        "tag": "my_safe",
...
def test_swap_tokens_success(client):
    ...
    response = client.post("/api/swap", json={
...
def test_get_swap_max_amount_sell_mode(client):
    ...
    response = client.get("/api/swap/max-amount?account=0x123&sell_token=WXDAI&buy_token=OLAS&mode=sell&chain=gnosis")
    assert response.status_code == 200
    data = response.json()
...
def test_swap_request_validation():
    ...
    from iwa.web.server import SwapRequest
...
def test_get_olas_services_basic_no_plugin(client):
    ...
    with patch("iwa.core.models.Config") as mock_config:
...
def test_get_olas_services_basic_with_services(client):
    ...
    with patch("iwa.core.models.Config") as mock_config:
        ...
        mock_config.return_value.plugins = {
            "olas": {"services": {"gnosis:1": mock_service}}
        }

        wallet.key_storage.find_stored_account = MagicMock(return_value=None)

        with patch("iwa.plugins.olas.models.OlasConfig") as mock_olas_cfg:
...
def test_get_olas_services_basic_import_error(client):
    ...
    with patch("iwa.core.models.Config", side_effect=ImportError("No module")):
        response = client.get("/api/olas/services/basic?chain=gnosis")
        assert response.status_code == 200
        assert response.json() == []
...
def test_get_swap_quote_error(client):
    ...
    response = client.get("/api/swap/quote?account=0x123&sell_token=WXDAI&buy_token=OLAS&amount=1&mode=sell&chain=gnosis")
    assert response.status_code == 400
...
def test_get_swap_quote_no_liquidity(client):
    ...
    with patch("iwa.web.server.ChainInterfaces") as mock_chains:
        ...
        with patch("iwa.plugins.gnosis.cow.CowSwap") as mock_cow:
            mock_cow.return_value.get_max_buy_amount_wei = AsyncMock(
...
def test_get_swap_max_amount_buy_mode_no_signer(client):
    ...
    response = client.get("/api/swap/max-amount?account=0x123&sell_token=WXDAI&buy_token=OLAS&mode=buy&chain=gnosis")
    assert response.status_code == 400
...
def test_verify_auth_no_password():
    ...
    from iwa.web.server import verify_auth
...
def test_transaction_request_amount_too_large():
